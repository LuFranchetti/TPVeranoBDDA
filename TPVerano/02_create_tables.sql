...
-- ==============================
IF OBJECT_ID('tempdb..#MermasRaw') IS NULL
BEGIN
...
-- ==============================
-- 17. Errores MERMA Temporal 
--Registra errores detectados durante el procesamiento.No afecta la tabla productiva.
-- ==============================
IF OBJECT_ID('tempdb..#ErroresMermas') IS NULL
BEGIN
    CREATE TABLE #ErroresMermas (
...
-- ==============================
-- 18. LogImportacionMermas Temporal
--Registra el log de las importaciones.
-- ==============================
IF OBJECT_ID('tempdb..#LogImportacionMermas') IS NULL
BEGIN
    CREATE TABLE #LogImportacionMermas(
...
GO
-- ==============================
-- 20. EstimacionesRaw Temporal
--Es la zona sucia. Aca entra el archivo con BULK INSERT.No tiene claves, no tiene restricciones.Es temporal.
-- ==============================
IF OBJECT_ID('tempdb..#EstimacionesRaw') IS NULL
BEGIN
...
-- ==============================
-- 17. Errores Estimaciones Temporal
--Registra errores detectados durante el procesamiento.No afecta la tabla productiva.
-- ==============================
IF OBJECT_ID('tempdb..#ErroresEstimaciones') IS NULL
BEGIN
    CREATE TABLE #ErroresEstimaciones (
        fecha DATETIME DEFAULT GETDATE(),
        descripcion VARCHAR(500),
        cultivo VARCHAR(200),
        campania VARCHAR(50),
...
-- ==============================
-- 20.LogImportacionEstimaciones Temporal
--Es la zona sucia. Aca entra el archivo con BULK INSERT.No tiene claves, no tiene restricciones.Es temporal.
-- ==============================
IF OBJECT_ID('tempdb..#LogImportacionEstimaciones') IS NULL
BEGIN
    CREATE TABLE #LogImportacionEstimaciones(
        id_log INT IDENTITY PRIMARY KEY,
        fecha_importacion DATETIME DEFAULT GETDATE(),
        registros_staging INT,
...
-- ==============================
-- 20. CapacitadoresRaw Temporal
--Es la zona sucia. Aca entra el archivo con BULK INSERT.No tiene claves, no tiene restricciones.Es temporal.
-- ==============================
IF OBJECT_ID('tempdb..#CapacitadoresRaw') IS NULL
BEGIN
...
-- ==============================
-- 17. Errores Capacitadores Temporal
--Registra errores detectados durante el procesamiento.No afecta la tabla productiva.
-- ==============================
IF OBJECT_ID('tempdb..#ErroresCapacitadores') IS NULL
BEGIN
    CREATE TABLE #ErroresCapacitadores (
        fecha DATETIME DEFAULT GETDATE(),
        descripcion VARCHAR(500),
        numero_registro VARCHAR(100)
    );
...
-- ==============================
-- 20. LogImportacionCapacitadores Temporal
--Es la zona sucia. Aca entra el archivo con BULK INSERT.No tiene claves, no tiene restricciones.Es temporal.
-- ==============================
IF OBJECT_ID('tempdb..#LogImportacionCapacitadores') IS NULL
BEGIN
    CREATE TABLE #LogImportacionCapacitadores(
        id_log INT IDENTITY PRIMARY KEY,
        fecha_importacion DATETIME DEFAULT GETDATE(),
        registros_staging INT,
        registros_actualizados INT,
...
-- ==============================
IF OBJECT_ID('tempdb..#PrecioMayorista') IS NULL
BEGIN
    CREATE TABLE #PrecioMayorista (
        ...
        -- Si quisieras, aquí también podrías definir la UNIQUE constraint
        -- CONSTRAINT UQ_precio UNIQUE (fecha, tipo_producto, especie, variedad, procedencia, tamanio, grado)
    );
END
GO
...
-- ==============================
-- 22. PreciosRaw Temporal
-- ==============================
IF OBJECT_ID('tempdb..#PreciosRaw') IS NULL
BEGIN
...
-- ==============================
-- 23. ErroresPrecios Temporal
-- ==============================
IF OBJECT_ID('tempdb..#ErroresPrecios') IS NULL
BEGIN
    CREATE TABLE #ErroresPrecios (
        fecha DATETIME DEFAULT GETDATE(),
        descripcion VARCHAR(500),
        especie VARCHAR(200),
...
-- ==============================
-- 24. LogImportacionPrecios Temporal
-- ==============================
IF OBJECT_ID('tempdb..#LogImportacionPrecios') IS NULL
BEGIN
    CREATE TABLE #LogImportacionPrecios(
        id_log INT IDENTITY PRIMARY KEY,
        fecha_importacion DATETIME DEFAULT GETDATE(),
        fecha_archivo DATE,
